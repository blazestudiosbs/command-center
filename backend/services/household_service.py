...
def list_pending_voice_identities() -> list[dict]:
    with connection() as conn:
        rows = conn.execute(
            """
            SELECT id,provider,substr(subject_hash,1,12) AS fingerprint,first_seen_utc,last_seen_utc,seen_count
            FROM pending_household_voice_identities
...
def link_pending_voice_identity(*, member_id: str, pending_id: str) -> dict:
    ensure_owner_member()
    with connection() as conn:
        member = conn.execute("SELECT * FROM household_members WHERE id=? AND status='active'", (member_id,)).fetchone()
        if not member or not member["user_id"]:
            raise ValueError("That household member is unavailable or does not have a private Vera account.")
        pending = conn.execute(
            "SELECT provider,subject_hash FROM pending_household_voice_identities WHERE id=?",
            (pending_id,),
        ).fetchone()
        if not pending:
            raise ValueError("That pending voice identity is unavailable.")
        identity_id = str(uuid.uuid4())
        conn.execute(
            f"""
            INSERT INTO household_voice_identities (id,household_member_id,provider,subject_hash,created_utc,updated_utc)
            ...
            ON CONFLICT(provider,subject_hash) DO UPDATE SET
                ...
                updated_utc=excluded.updated_utc
            """,
            (identity_id, member_id, pending["provider"], pending["subject_hash"]),
        )
        conn.execute("DELETE FROM pending_household_voice_identities WHERE id=?", (pending_id,))
        row = conn.execute(
            "SELECT id,household_member_id,provider,created_utc,updated_utc FROM household_voice_identities WHERE provider=? AND subject_hash=?",
            (pending["provider"], pending["subject_hash"]),
        ).fetchone()
    return dict(row)


def _record_pending_voice_identity(*, provider: str, subject_hash_value: str) -> None:
    with connection() as conn:
        conn.execute(
            f"""
            INSERT INTO pending_household_voice_identities
                (id,provider,subject_hash,first_seen_utc,last_seen_utc,seen_count)
            VALUES (?,?,?,{_utc_sql()},{_utc_sql()},1)
            ON CONFLICT(provider,subject_hash) DO UPDATE SET
                last_seen_utc={_utc_sql()},
                seen_count=seen_count+1
            """,
            (str(uuid.uuid4()), provider, subject_hash_value),
        )
...
def resolve_voice_identity(*, provider: str, subject_id: str) -> dict:
    ...
    if not row:
        _record_pending_voice_identity(provider=normalized_provider, subject_hash_value=hashed)
