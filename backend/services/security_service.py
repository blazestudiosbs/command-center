...
def _rule_source_category(source: str) -> str:
    ...
    try:
        ...
        if any(network.subnet_of(lan) or network.overlaps(lan) for lan in LAN_NETWORKS):
            return "lan"
    except Exception:
        pass
...
def _address_category(address: str, port: int, ufw_categories: dict[int, set[str]], firewall_enabled: bool) -> str:
    ...
    categories = ufw_categories.get(port, set())
    if firewall_enabled and categories and "public" not in categories and "lan" in categories:
        return "lan"

    return "public"


def get_listening_tcp_ports(ufw_status: dict[str, Any]) -> list[dict[str, Any]]:
    result = _run_cmd(["ss", "-ltnp"])
    if not result.get("ok"):
        result = _run_cmd(["netstat", "-ltnp"])

    ufw_categories = _ufw_port_categories(ufw_status)
    ...
    for line in result.get("stdout", "").splitlines():
        ...
        category = _address_category(address, port, ufw_categories, firewall_enabled)
        process = _extract_process(line)
        key = (port, category, process)
    ...
    return sorted(ports_by_key.values(), key=lambda item: (item["port"], item["category"], item["process"]))
...
def docker_ports_as_listeners(docker_published_ports: list[dict[str, Any]], ufw_status: dict[str, Any]) -> list[dict[str, Any]]:
    ufw_categories = _ufw_port_categories(ufw_status)
    firewall_enabled = bool(ufw_status.get("enabled"))
    listeners = []
    ...
    for published in docker_published_ports:
        ...
        listeners.append({
            "port": host_port,
            "protocol": "tcp",
            "category": _address_category(host_ip, host_port, ufw_categories, firewall_enabled),
            "addresses": [host_ip],
...
def _dedupe_ports(entries: list[dict[str, Any]], category: str) -> list[dict[str, Any]]:
    ...
    for entry in entries:
        if entry.get("category") != category:
            continue
        port = entry["port"]
        current = by_port.setdefault(port, {"port": port, "protocol": "tcp", "processes": set(), "addresses": set()})
...
def get_ssh_status(public_ports: list[dict[str, Any]], lan_ports: list[dict[str, Any]], localhost_ports: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {"running": sshd_running or public or lan or localhost, "exposure": exposure}
...
def build_recommendations(firewall_enabled: bool, default_policy: str, public_ports: list[dict[str, Any]], lan_ports: list[dict[str, Any]], localhost_ports: list[dict[str, Any]], ssh_status: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if firewall_enabled:
        ...
        _add_recommendation(recommendations, "problem", "Firewall disabled.", "Enable UFW and deny unsolicited inbound traffic by default.")

    if RCON_PORT in public_port_numbers:
        _add_recommendation(recommendations, "problem", "RCON publicly exposed.", "Move Minecraft RCON to localhost or LAN-only immediately.")
    elif RCON_PORT in localhost_port_numbers or RCON_PORT in lan_port_numbers:
        _add_recommendation(recommendations, "good", "RCON not publicly exposed.", "Minecraft RCON is not categorized as public.")
    else:
        _add_recommendation(recommendations, "recommendation", "RCON listener not detected.", "Confirm Minecraft RCON is only bound to localhost when enabled.")

    if COMMAND_CENTER_API_PORT in public_port_numbers:
        _add_recommendation(recommendations, "problem", "Command Center API publicly exposed.", "Limit port 8787 to LAN or localhost.")
    elif COMMAND_CENTER_API_PORT in lan_port_numbers or COMMAND_CENTER_API_PORT in localhost_port_numbers:
        _add_recommendation(recommendations, "good", "Command Center API limited to LAN.", "Port 8787 is not categorized as public.")
    else:
        _add_recommendation(recommendations, "recommendation", "Command Center API listener not detected.", "Confirm the API is reachable only from trusted networks when running.")

    if ssh_status.get("exposure") == "public":
        _add_recommendation(recommendations, "problem", "SSH exposed publicly.", "Restrict SSH to LAN/VPN and use key-only authentication.")
    elif ssh_status.get("running"):
        _add_recommendation(recommendations, "good", "SSH not publicly exposed.", f"SSH exposure is {ssh_status.get('exposure')}.")
    else:
    ...
    if unknown_public:
        _add_recommendation(recommendations, "problem", "Unknown public ports detected.", f"Review public ports: {', '.join(str(port) for port in unknown_public)}.")
    else:
        _add_recommendation(recommendations, "good", "No unknown public ports detected.", "Public listeners are limited to known service ports.")
...
def calculate_score(firewall_enabled: bool, public_ports: list[dict[str, Any]], ssh_status: dict[str, Any], recommendations: list[dict[str, str]]) -> int:
    ...
    score -= min(15, sum(1 for rec in recommendations if rec["severity"] == "recommendation") * 3)
...
def get_status() -> dict[str, Any]:
    ...
    listening_ports = get_listening_tcp_ports(ufw_status)
    docker_published_ports = get_docker_published_ports()
    categorized_ports = listening_ports + docker_ports_as_listeners(docker_published_ports, ufw_status)
    public_ports = _dedupe_ports(categorized_ports, "public")
    lan_ports = _dedupe_ports(categorized_ports, "lan")
    localhost_ports = _dedupe_ports(categorized_ports, "localhost")
    ssh_status = get_ssh_status(public_ports, lan_ports, localhost_ports)
    firewall_enabled = bool(ufw_status.get("enabled"))
